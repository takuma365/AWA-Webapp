...
HTML_TAGS = {
    ...
    'marker_template': '<span class="marker"><strong>{content}</strong></span>',#黄色マーカー
...
ID_PATTERNS = {
    ...
    'link_item_format': 'heading_{main_number}-{sub_number}',
}
        ...
        if tag == 'p':
            ...
            if pStyle is not None and pStyle.get('{' + namespaces['w'] + '}val') == '1':
                ...
                if text_content:
                    ...
                    heading_html = f'{HTML_TAGS["nbsp_paragraph"]}\n\n{HTML_TAGS["nbsp_paragraph"]}\n\n{heading_html_content}\n'
            ...
            # パラグラフ内の青色テキストとコメントのURLをリンクとして処理
                ...
                if blue_text_segments and comment_urls:
                    ...
                    html_elements.append(HTML_TAGS['paragraph_template'].format(content=html_content))
                else:
                    # 青色テキストかURLがない場合は通常のテキストとして処理
                    formatted_content = process_paragraph_runs(p, namespaces)
                    if formatted_content:
                        html_elements.append(HTML_TAGS['paragraph_template'].format(content=formatted_content))
            ...
            # 罫線内の青色テキストの場合は何もしない（見出し直後のリンクリストに任せる）
                ...
                if text_content and text_content.strip().startswith('・'):
                    ...
                    heading_id = ID_PATTERNS['link_item_format'].format(main_number=heading_counters[1], sub_number=heading_counters['link_counter'])
...
def configure_from_json_data(json_data):
    ...
    if not conversion_settings:
        print("[WARNING] conversion_settingsが見つかりません")
        return
    ...
    for rule in rules:
        ...
        if section in section_mapping:
            ...
            if section == '太字':
                ...
                processed_tag = re.sub(r'^<p[^>]*>(.*)</p>$', r'\1', processed_tag)
            
            # ハイライトの場合も同様にpタグを除去
            elif section == 'ハイライト':
                # <p><span class="marker"><strong>{content}</strong></span></p> → <span class="marker"><strong>{content}</strong></span>
                processed_tag = re.sub(r'^<p[^>]*>(.*)</p>$', r'\1', processed_tag)
...
def process_bullet_list_items(items, list_template):
    """
    「・」から始まるテキスト項目を動的なHTMLリスト構造に変換する
    
    Args:
        items (list): 「・」から始まるテキスト項目のリスト
    ...
    if has_content_placeholders:
        # ul/li形式の場合
        if '<ul>' in list_template and '<li>' in list_template:
            ...
            if li_matches and len(li_matches) > 1:
                ...
                if li_content_match:
                    ...
                    # 外側の構造を抽出
                    outer_start = list_template.split('<ul')[0] if '<ul' in list_template else ''
                    ul_start_match = re.search(r'<ul[^>]*>', list_template)
                    ul_start = ul_start_match.group(0) if ul_start_match else '<ul>'
                    outer_end_match = re.search(r'</ul>(.*?)$', list_template, re.DOTALL)
